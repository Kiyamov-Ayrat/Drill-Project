...
async def create_conductor(session: SessionDep, well_id: int,
...
@router.get("/well_columns/{well_id}",
            ...
            tags=["well_construction"])
async def get_columns(session: SessionDep, well_id: int) -> list[ColumnResponse]:
    well = await session.get(Well, well_id)
    if well is None:
        raise HTTPException(status_code=404, detail="Well not found")
...
@router.post("/well_columns",
             ...
             tags=["well_construction"])
async def create_well_columns(session: SessionDep, well_id: int):
    well = await session.get(Well, well_id)
    if well is None:
        raise HTTPException(status_code=404, detail="Well not found")
    stmt = select(Info).where(Info.well_id==well_id).order_by(Info.depth_up_m)
    result = await session.scalars(stmt)
    intervals_db = result.all()
    if not intervals_db:
        return []
...
@router.post("/well_construction",
             response_model=list[CasingDepth],
             tags=["well_construction"])
async def create_well_construction(session: SessionDep, well_id: int,
...
@router.delete("/well_columns/{well_id}/{column_name}",
               tags=["well_construction"])
async def delete_column(session: SessionDep, well_id: int, column_name: str):
    well = await session.get(Well, well_id)
    if well is None:
        raise HTTPException(status_code=404, detail="Well not found")
